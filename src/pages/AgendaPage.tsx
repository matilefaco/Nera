...
import {
  ...
  sanitizeAppointment,
  rescheduleBookingByProfessional,
} from "../firebase";
import {
  collection,
  query,
...
import {
  getAvailableSlots as getAvailableSlotsOld,
  getDayAvailability,
} from "../lib/bookingUtils";
import {
  getAvailableSlots,
  IntelligentFit,
} from "../utils/scheduleSuggestions";
...
const blockedSchedulesCache = new Map<
  string,
...
function formatTimelineDate(dateStr: string): string {
  try {
    ...
    return formatLocalDate(formatDateKey(d), {
      day: "2-digit",
      month: "short",
    });
  } catch {
...
export default function AgendaPage() {
  ...
  useEffect(() => {
    if (appointmentIdFromUrl && user) {
      const fetchAppt = async () => {
        try {
          const apptSnap = await getDoc(
            doc(db, "appointments", appointmentIdFromUrl),
          );
          if (apptSnap.exists()) {
  ...
  const [notifyingWaitlist, setNotifyingWaitlist] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (isMounted)
        setAllAppointmentsStatus((prev) =>
          prev === "loading" ? "stalled" : prev,
        );
    }, 2000);
    ...
    const unsubAll = onSnapshot(
      ...
      (error) => {
        ...
        if (isDev)
          console.error(
            "[AgendaPage] Subscription error (allAppointments):",
            error,
          );
        setAllAppointmentsStatus("error");
        ...
        if (error.message.includes("index")) {
          if (isDev)
            console.log(
              "[AgendaPage] Firestore index required: appointments professionalId ASC, date ASC",
            );
  ...
  const recommendedSlots = React.useMemo(() => {
    if (!manualService || !manualDate || !profile?.workingHours)
      return { bestSlot: null, otherSlots: [], intelligentFits: [] };
  ...
  const availableRescheduleSlots = React.useMemo(() => {
    if (!rescheduleDate || !selectedAppointment || !profile?.workingHours)
      return [];
    ...
    const filteredAppts = allAppointments.filter(
      (a) => a.id !== selectedAppointment.id && a.date === rescheduleDate,
    );
    const dayOfWeek = parseLocalDate(rescheduleDate).getDay();
    const dayBlocks = allBlockedSchedules.filter(
      (b) =>
  ...
  const handleCreateManual = async () => {
    ...
    try {
      ...
      notify.success(
        `Agendamento de ${manualClient} criado para ${manualTime}.`,
      );
      setManualClient("");
      setManualPhone("");
      setManualService("");
  ...
  const handleComplete = async (app: any) => {
    ...
    try {
      ...
      notify.success(
        "Atendimento finalizado. A cliente receberá o link de avaliação por e-mail.",
      );
      setIsDetailsOpen(false);
    } catch (err) {
  ...
  const handleRespond = async (
    id: string,
    decision: "confirmed" | "cancelled_by_professional",
    appointment?: any,
  ) => {
      ...
      if (decision === "confirmed") {
        ...
        if (appointment?.status === "confirmed") {
          const res = await fetch(
            `/api/appointments/${id}/cancel-by-professional`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          if (!res.ok) {
            const errData = await res.json().catch(() => ({}));
            throw new Error(
              errData.error || `Erro ao cancelar (${res.status})`,
            );
          }
        } else {
  ...
  const confirmedAppts = displayedAppointments.filter(
  ...
  const timelineItems = React.useMemo(() => {
    ...
    displayedAppointments.forEach((app) => {
      ...
      items.push({
        ...
        status: app.status,
        hasConflict:
          (counts[app.time] > 1 && isActiveSlotStatus(app.status)) ||
    ...
    openSlots.forEach((slot) => {
      ...
      const hasStrictMeeting = displayedAppointments.some(
        (a) => a.time === slot && isActiveSlotStatus(a.status),
      );
        ...
        >
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col">
                ...
                {view === "day" && isSelectedDateToday && (
                  <span className="text-[10px] text-brand-terracotta font-bold uppercase tracking-[0.2em] mt-0.5 inline-block">
                    Hoje
                  </span>
                )}
              </div>
              ...
              <div className="flex bg-brand-linen/50 p-1.5 rounded-2xl text-[9px] font-bold uppercase tracking-widest border border-brand-mist/20">
                <button
                  onClick={() => setView("day")}
                  className={cn(
                    "flex-1 px-4 py-3 min-h-[44px] rounded-xl transition-all flex items-center justify-center",
                    view === "day"
            ...
            >
              ...
              <div>
                <h3 className="text-sm font-bold text-red-900 uppercase tracking-widest mb-1">
                  Atenção: Conflito detectado
                </h3>
                <p className="text-xs text-red-700 leading-relaxed font-medium">
                  Identificamos que existem {conflicts.length} horário(s) com
                  mais de um serviço confirmado simultaneamente. Isso pode ter
                  ocorrido por falhas em versões anteriores. Por favor, revise
                  os horários marcados em vermelho na timeline.
                </p>
              </div>
            </motion.div>
          )}
          ...
          <div className="bg-white/60 backdrop-blur-md border border-brand-mist/60 rounded-2xl p-1 mb-6 flex items-center gap-2.5 px-4 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.04)] focus-within:shadow-[0_4px_16px_-4px_rgba(0,0,0,0.06)] focus-within:border-brand-mist/80 transition-all duration-300">
            <Search
              size={14}
              strokeWidth={1.5}
              className="text-brand-stone/60 shrink-0"
            />
            <input
              type="text"
              value={searchCode}
          ...
          <div className="grid grid-cols-3 gap-2 mb-8">
            <div className="bg-white/30 backdrop-blur-md p-3.5 rounded-[20px] border border-brand-mist/20 text-center transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_8px_24px_-8px_rgba(0,0,0,0.03)] cursor-default">
              <p className="text-[9px] font-medium text-brand-stone/70 uppercase tracking-[0.2em] mb-1.5">
                Confirmados
              </p>
              <p className="text-[24px] font-serif text-brand-ink/85 leading-none">
            ...
            <div className="bg-white/70 backdrop-blur-xl p-3.5 rounded-[20px] border border-brand-mist/40 shadow-[0_8px_30px_-6px_rgba(0,0,0,0.05)] text-center relative overflow-hidden transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_32px_-6px_rgba(0,0,0,0.08)] cursor-default">
              <p className="text-[9px] font-medium text-brand-stone/80 uppercase tracking-[0.2em] mb-1.5">
                Pedidos
              </p>
              <p className="text-[24px] font-serif text-brand-ink/95 leading-none flex items-center justify-center gap-1.5">
                {pendingRequests.length}
                {pendingRequests.length > 0 && (
                  <span className="w-1 h-1 shadow-[0_0_8px_2px_rgba(202,106,86,0.2)] bg-brand-terracotta/90 rounded-full animate-pulse" />
                )}
              </p>
            </div>
            <div className="bg-white/30 backdrop-blur-md p-3.5 rounded-[20px] border border-brand-mist/20 text-center transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_8px_24px_-8px_rgba(0,0,0,0.03)] cursor-default">
              ...
              <div className="bg-red-50/50 border border-red-100 p-6 rounded-[32px]">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-[9px] font-bold uppercase tracking-widest text-red-600">
                    Pedidos aguardando ({pendingRequests.length})
                  </h4>
                  <Link
                    to="/agenda"
                    className="text-[9px] font-bold uppercase tracking-widest text-brand-ink underline"
                  >
                    Ver todos
                  </Link>
                </div>
            ...
            {openSlots.length > 0 && (
              <div className="bg-white/40 backdrop-blur-md p-6 rounded-[28px] border border-brand-mist/40 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.03)] cursor-default">
                <div className="mb-6 flex flex-col items-center text-center">
                  <h4 className="text-[9px] font-medium uppercase tracking-[0.2em] text-brand-stone/80 mb-3">
                    {openSlots.length} horários livres hoje
                  </h4>
                  <div className="flex flex-wrap justify-center items-center gap-x-3 gap-y-2 max-w-[220px]">
                    {openSlots.slice(0, 6).map((s, i) => (
                      <div key={i} className="flex items-center">
                        <span className="text-[14px] text-brand-ink/90 font-serif tracking-wide">
                          {s}
                        </span>
                        {i < Math.min(openSlots.length, 6) - 1 && (
                          <span className="ml-3 w-1 h-1 rounded-full bg-brand-stone/20"></span>
                        )}
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex gap-2.5">
                  ...
                  >
                    <Users
                      size={13}
                      strokeWidth={1.5}
                      className="text-brand-stone/80 transition-colors"
                    />{" "}
                    Ver espera
                  </button>
                </div>
            ...
            {isSelectedDateToday &&
              (confirmedAppts.length >= 5 ? (
                <div className="text-center space-y-1 relative z-10 before:absolute before:inset-0 before:-z-10 before:bg-gradient-to-b before:from-brand-linen/40 before:to-transparent before:opacity-0 before:transition-opacity">
                  <p className="text-[17px] font-serif text-brand-ink/90 italic tracking-tight">
                    Agenda cheia hoje ✨
                  </p>
                  <p className="text-[10px] font-medium text-brand-stone/60 uppercase tracking-[0.15em]">
                    {confirmedAppts.length} atendimentos confirmados
                  </p>
                </div>
              ) : (
        ...
        <div className="fixed bottom-[calc(92px+env(safe-area-inset-bottom))] right-5 z-[100] md:bottom-12 md:right-12">
          ...
          <button
            ...
            className={cn(
              ...
              isFabOpen
                ? "rotate-[135deg] bg-brand-terracotta/95 shadow-[0_12px_40px_rgba(202,106,86,0.3)]"
                : "hover:shadow-[0_16px_40px_rgb(0,0,0,0.2)]",
            )}
          >
            <Plus
              size={26}
              ...
              >
                ...
                {agendaView === "details" && (
                  ...
                  >
                    <div className="mb-5 sm:mb-6 pr-12 relative">
                      <span className="block text-[9px] font-bold uppercase tracking-[0.2em] text-brand-terracotta mb-2">
                        Detalhes da Reserva
                      </span>

                      <h3 className="text-2xl sm:text-3xl font-serif text-brand-ink leading-tight mb-1">
                        {selectedAppointment.clientName}
                      </h3>
                      <p className="text-sm text-brand-stone font-light italic mb-3">
                        {selectedAppointment.additionalServices?.length > 0
                          ? [selectedAppointment.serviceName, ...selectedAppointment.additionalServices.map((s:any) => s.name)].join(" • ")
                      ...
                      <div className="flex items-center flex-wrap gap-2">
                        ...
                        {selectedAppointment.reservationCode && (
                          <div className="inline-flex items-center gap-1.5 bg-brand-linen/60 px-2.5 py-1 rounded-full border border-brand-mist/30 leading-none">
                            <span className="text-[8px] text-brand-stone uppercase tracking-widest font-bold">
                              Cód:
                            </span>
                            <span className="text-[9px] font-mono font-bold text-brand-terracotta">
                              {selectedAppointment.reservationCode}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="p-5 sm:p-6 bg-brand-parchment/60 rounded-[24px] sm:rounded-[32px] border border-brand-mist/50 space-y-4 sm:space-y-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-base sm:text-lg font-serif text-brand-ink">
                              {formatLocalDate(selectedAppointment.date, {
                                day: "numeric",
                                month: "long",
                                year: "numeric",
                              })}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-brand-stone mb-0.5">
                              Serviço
                              {selectedAppointment.additionalServices?.length
                                ? "s"
                                : ""}
                            </p>
                            <p className="text-sm font-serif text-brand-ink">
                              ...
                              0
                                ? [
                                    selectedAppointment.serviceName,
                                    ...selectedAppointment.additionalServices.map(
                                      (s: any) => s.name,
                                    ),
                                  ].join(", ")
                                : selectedAppointment.serviceName}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            {selectedAppointment.travelFee > 0 && (
                              <div className="text-[10px] text-brand-stone mt-0.5 font-medium">
                                Inclui{" "}
                                {formatCurrency(selectedAppointment.travelFee)}{" "}
                                de deslocamento
                              </div>
                            )}
                              ...
                              selectedAppointment.discountAmount > 0 && (
                                <div className="flex flex-col mt-0.5">
                                  <span className="text-[10px] text-brand-terracotta border border-brand-terracotta/20 bg-brand-terracotta/5 px-2 py-0.5 rounded-full inline-block w-fit">
                                    Cupom {selectedAppointment.couponCode}{" "}
                                    aplicado
                                  </span>
                                  <span className="text-[10px] text-brand-stone font-medium mt-1">
                                    ...
                                    <span className="line-through">
                                      {formatCurrency(
                                        selectedAppointment.originalPrice || 0,
                                      )}
                                    </span>{" "}
                                    (Desconto:{" "}
                                    {formatCurrency(
                      ...
                      {selectedAppointment.locationType === "home" && (
                        <div className="p-5 sm:p-6 bg-brand-parchment/60 rounded-[24px] sm:rounded-[32px] border border-brand-mist/50">
                          <div className="flex items-start sm:items-center gap-4">
                            ...
                            <div>
                              <p className="text-[10px] font-bold uppercase tracking-widest text-brand-stone mb-0.5">
                                Localização
                              </p>
                              <p className="text-sm text-brand-ink leading-relaxed">
                                {typeof selectedAppointment.address ===
                                "object" ? (
                                  <>
                                    ...
                                    {selectedAppointment.address.reference && (
                                      <div className="text-[10px] text-brand-terracotta mt-1 leading-tight">
                                        Ref:{" "}
                                        {selectedAppointment.address.reference}
                                      </div>
                                    )}
                                  ...
                                  <>
                                    {selectedAppointment.address ||
                                      selectedAppointment.neighborhood ||
                                      "Endereço a combinar"}
                                    {(selectedAppointment.address ||
                                      selectedAppointment.neighborhood) && (
                                      <button
                      ...
                      {selectedAppointment.timeline &&
                        selectedAppointment.timeline.length > 0 && (
                          <div className="pt-4 pb-2">
                            <p className="text-[9px] font-bold uppercase tracking-widest text-brand-stone mb-4">
                              ...
                              {[...selectedAppointment.timeline]
                                ...
                                .map((event, idx) => (
                                  <div key={idx} className="relative pl-4">
                                    ...
                                    <p className="text-[11px] text-brand-ink font-medium leading-tight">
                                      {event.label}
                                    </p>
                                    <p className="text-[9px] text-brand-stone mt-0.5">
                ...
                {agendaView === "reschedule" && (
                  ...
                  >
                    ...
                    <h3 className="text-2xl font-serif text-brand-ink leading-tight mb-2">
                      Remarcar Horário
                    </h3>
                    <p className="text-sm text-brand-stone font-light mb-8">
                      Escolha a nova data e horário para{" "}
                      {selectedAppointment.clientName}. A cliente será
                    ...
                    <div className="space-y-6">
                      ...
                      {rescheduleDate && (
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-widest text-brand-stone mb-2 text-center">
                            Horários Disponíveis
                          </label>
                          {availableRescheduleSlots.length > 0 ? (
                            ...
                            <div className="p-6 bg-brand-linen/20 rounded-[28px] border border-dashed border-brand-mist/50 flex flex-col items-center text-center gap-2">
                              ...
                              <p className="text-[10px] text-brand-stone font-light px-4">
                                Nenhum horário disponível para este serviço em{" "}
                                {formatLocalDate(rescheduleDate, {
                                  day: "2-digit",
                                  month: "2-digit",
                                })}
                                .
