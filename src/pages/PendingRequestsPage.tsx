...
export default function PendingRequestsPage() {
  ...
  const [pushBannerDismissed, setPushBannerDismissed] = useState(() => {
    return (
      localStorage.getItem("nera_push_banner_dismissed_pending") === "true"
    );
  });
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        try {
          ...
          if (isDev)
            console.error(
              "[PendingRequestsPage] Error parsing snapshot callback:",
              err,
            );
        }
      },
      (error) => {
        if (isDev)
          console.error(
            "[PendingRequestsPage] Firestore onSnapshot error:",
            error,
          );
        setLoading(false);
      },
  ...
  useEffect(() => {
    setLocalRequests((prev) => {
      ...
      const heldItem = confirmedId
        ? prev.find((p) => p.id === confirmedId)
        : null;
      ...
      if (
        heldItem &&
        !currentIds.has(heldItem.id) &&
        (heldItem.id === confirmedId || !handledIds.includes(heldItem.id))
      ) {
  ...
  const handleRespond = async (
    ...
    decision:
      ...
      | typeof APPOINTMENT_STATUS.CANCELLED_BY_PROFESSIONAL,
    appointment?: any,
  ) => {
      ...
      <div className="p-4 md:p-16 max-w-5xl mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <span className="text-[10px] font-bold text-brand-terracotta uppercase tracking-[0.3em] block">
                  Pedidos Pendentes
                </span>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h4 className="text-base font-serif text-brand-white">
                    Não perca nenhum pedido
                  </h4>
                  <p className="text-xs text-brand-parchment/60 font-light italic mt-1 leading-relaxed">
                    Ative as notificações para ser avisada no celular quando
                    chegar uma nova reserva.
                  </p>
                </div>
              </div>
                ...
                <button
                  onClick={() => {
                    ...
                    localStorage.setItem(
                      "nera_push_banner_dismissed_pending",
                      "true",
                    );
                  }}
        ...
        {loading ? (
          ...
          <div className="bg-brand-white border border-brand-mist/60 border-dashed rounded-[48px] p-16 md:p-24 text-center flex flex-col items-center max-w-xl mx-auto shadow-sm">
            ...
            <h2 className="text-2xl font-serif text-brand-ink mb-3 text-balance">
            ...
            <Link
              to="/dashboard"
              className="px-10 py-5 bg-brand-ink text-brand-white rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-brand-espresso transition-all shadow-sm"
            >
              Voltar ao Início
            </Link>
          </div>
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      !confirmedId &&
                        timeInfo?.isVeryNew &&
                        "shadow-brand-terracotta/5 border-brand-terracotta/10",
                      !confirmedId &&
                        !timeInfo?.isVeryNew &&
                        "border-brand-mist/30",
                    )}
                  >
                    {/* Glow for very new */}
                    ...
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="text-right flex flex-col items-end">
                        <p className="text-xl md:text-2xl font-bold text-brand-ink">
                          {formatCurrency(
                            request.finalPrice ??
                              request.totalPrice ??
                              request.price,
                        ...
                        {request.travelFee > 0 && (
                          <div className="text-[9px] text-brand-stone font-medium mt-0.5">
                            Inclui {formatCurrency(request.travelFee)} desloc.
                          </div>
                        )}
                        {request.couponCode && request.discountAmount > 0 ? (
                          ...
                          <p className="text-[8px] text-brand-stone uppercase tracking-widest font-bold opacity-60">
                            Valor
                          </p>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-4 mb-6">
                      <div className="bg-brand-parchment/60 rounded-3xl p-4 border border-brand-mist/30 flex items-center gap-3 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-[10px] text-brand-stone uppercase tracking-widest scale-90 origin-left truncate">
                            Data
                          </span>
            ...
            >
              <div className="p-8 md:p-14">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    <span className="text-[10px] font-bold text-brand-terracotta uppercase tracking-[0.3em] mb-4 block">
                      Detalhes do Pedido
                    </span>
                  ...
                  <button
                    onClick={() => setIsModalOpen(false)}
                    className="p-4 bg-brand-linen text-brand-stone hover:text-brand-ink rounded-[24px] transition-colors"
                  >
                    <X size={20} />
                  </button>
                </div>
                  ...
                  <div className="space-y-10">
                      ...
                      <div className="p-8 bg-brand-parchment/50 rounded-[32px] border border-brand-mist/40">
                        ...
                        <div className="flex items-center justify-between mt-6">
                          ...
                          <div className="flex flex-col items-end">
                              ...
                              selectedRequest.discountAmount > 0 && (
                                <div className="flex items-center gap-1.5 mt-1">
                                  <span className="text-[10px] text-brand-stone line-through">
                                    De{" "}
                                    {formatCurrency(
                                      selectedRequest.originalPrice || 0,
                                    )}
                                  </span>
                                  <span className="text-[9px] text-brand-terracotta bg-brand-terracotta/5 border border-brand-terracotta/20 px-1.5 py-0.5 rounded font-bold uppercase tracking-widest">
                            ...
                            {selectedRequest.travelFee > 0 && (
                              <span className="text-[10px] text-brand-stone font-medium mt-1">
                                (Inclui {formatCurrency(selectedRequest.travelFee)} func. domicílio)
                              </span>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-6">
                      <div className="space-y-2 min-w-0">
                        <p className="text-[9px] text-brand-stone uppercase tracking-widest font-bold opacity-60">
                          Data Escolhida
                        </p>
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="p-7 bg-brand-white rounded-[32px] border border-brand-mist/60 shadow-sm flex items-start gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="text-[11px] font-bold text-brand-ink uppercase tracking-widest mb-1">
                            {selectedRequest.locationType === "home"
                            ...
                            {selectedRequest.locationType === "home" ? (
                              selectedRequest.address && typeof selectedRequest.address === "object" ? (
                                <>
                                  ...
                                  {selectedRequest.address.complement
                                    ? ` - ${selectedRequest.address.complement}`
                                    : ""}{" "}
                                  - {selectedRequest.address.neighborhood},{" "}
                                  ...
                                  {selectedRequest.address.reference && (
                                    <div className="text-[10px] text-brand-terracotta mt-1 leading-tight">
                                      Ref: {selectedRequest.address.reference}
                                    </div>
                                  )}
                                  <button
                                    onClick={() => {
                                ...
                                <>
                                  ...
                                  {(selectedRequest.address ||
                                    ...
                                    <button
                                      onClick={() => {
                                        const addr =
                                          (selectedRequest.address as any) ||
                                          selectedRequest.neighborhood;
                                        window.open(
                                          `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(addr)}`,
                                          "_blank",
                  ...
                  <div className="space-y-10">
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="space-y-3">
                        <a
                          href={buildWhatsappLink(
                            selectedRequest.clientWhatsapp,
                          )}
                          target="_blank"
                          rel="noopener noreferrer"
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-[9px] font-bold text-brand-stone uppercase tracking-widest leading-none mb-1.5">
                                Enviar WhatsApp
                              </p>
                          ...
                          <ChevronRight
                            size={18}
                            className="text-brand-stone group-hover:text-brand-terracotta group-hover:translate-x-1 transition-all"
                          />
                        </a>
                        ...
                        {selectedRequest.clientEmail && (
                          <div className="p-6 bg-brand-parchment/30 rounded-[28px] border border-brand-mist/40 flex items-center gap-4">
                            ...
                            <div className="min-w-0">
                              <p className="text-[9px] font-bold text-brand-stone uppercase tracking-widest mb-1 opacity-60">
                                E-mail de Cadastro
                              </p>
                              <p className="text-[13px] font-medium text-brand-ink truncate">
                                {selectedRequest.clientEmail}
                              </p>
                            </div>
                          </div>
        ...
        {isConfirmRejectOpen && requestToReject && (
          <div className="fixed inset-0 z-[600] flex items-center justify-center p-6">
            ...
            >
              ...
              <h3 className="text-2xl font-serif text-brand-ink mb-4">
                Cancelar pedido?
              </h3>
              <p className="text-sm text-brand-stone font-light italic mb-10 leading-relaxed px-4">
                Ao cancelar, a cliente será notificada que você não poderá
                atendê-la neste horário.
              </p>
