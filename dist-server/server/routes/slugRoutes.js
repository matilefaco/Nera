...
router.get("/check", async (req, res) => {
    try {
        ...
        const db = getDb();
        if (!db) {
            logger.error("SERVER", "Firestore db is NOT initialized in slugRoutes", { requestId: req.requestId });
        ...
        try {
            ...
            logger.error("SERVER", "Slug check failed inside db operations", { error: innerErr.message, requestId: req.requestId });
            return res.status(500).json({
        ...
        logger.error("SERVER", "Slug check outer error", { error: err.message, requestId: req.requestId });
        return res.status(500).json({
