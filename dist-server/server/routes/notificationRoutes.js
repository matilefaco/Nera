...
if (VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY) {
    webpush.setVapidDetails('mailto:contato@usenera.com', VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY);
}
export async function sendPushToUser(professionalId, payload) {
    const db = getDb();
    try {
 ...
 */
async function createLastMinuteAlert(payload) {
    const db = getDb();
...
router.get("/test-whatsapp", debugOnly, async (req, res) => {
    ...
    if (simulateInbound === 'true') {
        const result = await handleInboundMessage(db, phone, message || '1', { simulated: true });
        return res.json(result);
    }
    const targetPhone = phone;
    const eventType = type || 'test_generic';
    let msg = message;
    const mockData = {
...
router.post("/notify", checkPlanFeature('whatsappNotifications'), async (req, res) => {
    ...
    try {
        ...
        if (type === 'BOOKING_CANCELLED' || type === 'BOOKING_CANCELLED_BY_CLIENT') {
            ...
            if (proDoc.exists) {
                ...
                if (pro?.email && await shouldSendEmail(appointmentId, eventKeyPro)) {
                    ...
                    if (result.success)
                        await markEmailSent(appointmentId, eventKeyPro);
                }
                if (clientWhatsapp) {
                    const formattedDate = date.split('-').reverse().join('/');
                ...
                if (clientEmail && await shouldSendEmail(appointmentId, eventKeyClient)) {
                    ...
                    if (result.success)
                        await markEmailSent(appointmentId, eventKeyClient);
                }
            }
            return res.json({ success: true });
        }
        ...
        if (type === 'BOOKING_RESCHEDULED_BY_CLIENT' || type === 'BOOKING_RESCHEDULED') {
            const { professionalId, clientName, clientEmail, previousDate, previousTime, date, time, appointmentId, serviceName, rescheduledBy: payloadRescheduledBy } = payload;
