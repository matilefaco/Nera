...
// 1. Get Auth URL
router.get("/auth-url", requireFirebaseAuth, (req, res) => {
    const uid = req.uid;
    const professionalIdQuery = req.query.professionalId;
 ...
 */
export async function createGoogleCalendarEvent(appointment, professionalId) {
    const db = getDb();
    try {
        const userDoc = await db.collection("users").doc(professionalId).get();
        const integration = userDoc.data()?.integrations?.google_calendar;
