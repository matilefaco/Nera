...
const getClientKey = (phone, email, name) => {
...
function getBookingLockId(appointment) {
    ...
    if (!appointment?.professionalId || !dateAttr || !timeAttr)
        return null;
...
router.post("/public/create-booking", bookingRateLimiter, async (req, res) => {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            const existingApptsSnap = await transaction.get(db.collection('appointments')
        ...
        (async () => {
            try {
                ...
                let paymentMethodsArr = Array.isArray(appointmentData.paymentMethods)
...
router.get("/debug-confirmation-email", debugOnly, async (req, res) => {
    try {
        ...
        if (!data?.clientEmail)
            result.reason += "Missing clientEmail. ";
        if (!data?.token)
...
router.get("/run-confirmation-email", debugOnly, async (req, res) => {
    ...
    try {
        ...
        if (!proId)
            throw new Error("Missing professionalId in appointment");
        ...
        if (!payload.token)
            throw new Error("Validation Failed: Missing token");
...
router.get("/debug-slot-lock", debugOnly, async (req, res) => {
    try {
        ...
        const activeApps = allApps.filter((a) => isActiveSlotStatus(a.status) || a.status === 'pending_conflict');
        const pendingApps = allApps.filter((a) => isPendingStatus(a.status));
        res.json({
            lockId,
...
router.post("/appointments/:appointmentId/complete", requireFirebaseAuth, async (req, res) => {
    const db = getDb();
    const { appointmentId } = req.params;
    const uid = req.uid;
    try {
...
router.get("/debug-can-book-slot", debugOnly, async (req, res) => {
    try {
        ...
        if (!professionalId || !date || !time)
            return res.status(400).json({ error: "Missing professionalId, date or time" });
        const proDoc = await db.collection('users').doc(professionalId).get();
        if (!proDoc.exists)
...
router.get("/debug-reservation-token", debugOnly, async (req, res) => {
    try {
        ...
        const results = { tokenReceived: token };
        // Search by token
        ...
        // Search by docId
        const q4 = await db.collection('appointments').doc(token).get();
...
router.get("/debug-bookable-slots", debugOnly, async (req, res) => {
    try {
        ...
        res.json({
            ...
            appointments: appointments.map((a) => ({ time: a.time, duration: a.duration })),
            blocks: dayBlocks.map((b) => ({ start: b.startTime, end: b.endTime }))
        });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.post("/public/manage/:manageSlug/cancel", async (req, res) => {
    const db = getDb();
    const { manageSlug } = req.params;
