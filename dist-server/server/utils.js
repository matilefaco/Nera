...
export function formatBRNumber(phone) {
 ...
 */
export async function shouldSendEmail(appointmentId, eventKey) {
    if (!appointmentId)
...
export async function markEmailSent(appointmentId, eventKey) {
    if (!appointmentId)
        return;
    const db = getDb();
    try {
...
export async function sendWhatsAppMeta(to, message, metadata = {}) {
    const db = getDb();
...
export async function callNvidiaAI(messages, options = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
            ...
            if (attempt === 0)
                console.log(`[NVIDIA] Retrying...`);
        }
    }
    console.error(`[NVIDIA] final failure:`, lastError.message);
    throw lastError;
}
export async function getServiceDescriptionWithFallback(serviceName, specialty, duration, price, tone) {
    const nvidiaKey = process.env.NVIDIA_API_KEY;
