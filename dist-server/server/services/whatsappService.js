 ...
 */
export function validateBrazilPhone(phone) {
    const normalized = normalizePhone(phone);
 ...
 */
export async function logWhatsAppMessage(_db, data) {
    ...
    if (logId) {
        ...
        if (data.zapiResponse)
            updateData.zapiResponse = data.zapiResponse;
        if (data.metaResponse)
            updateData.metaResponse = data.metaResponse;
        ...
        if (data.status === 'sent')
            logData.sentAt = admin.firestore.FieldValue.serverTimestamp();
        if (data.error)
 ...
 */
export async function sendWhatsApp(_db, phone, message, metadata) {
    const db = getDb();
...
export function detectWhatsAppIntent(message) {
    ...
    if (confirmPatterns.some(p => m === p || m.includes(p)))
        return 'confirm';
    if (reschedulePatterns.some(p => m === p || m.includes(p)))
        return 'reschedule';
    if (cancelPatterns.some(p => m === p || m.includes(p)))
        return 'cancel';
    if (helpPatterns.some(p => m === p || m.includes(p)))
        return 'help';
    return 'unknown';
}
/**
 ...
 */
export async function handleInboundMessage(_db, phone, message, rawPayload) {
    const db = getDb();
    const normalizedPhone = normalizePhone(phone);
 ...
 * Admin test function
 */
export async function sendTestWhatsApp(db, phone, message) {
